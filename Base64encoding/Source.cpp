...
int main( ) {
    for( size_t i = 1000; i < 1010; i++ )
    {
        ...
        std::string sample = "Dhananjay";

        //const unsigned char* bytes = ( const unsigned char* )( &num );
        const unsigned char* bytes = reinterpret_cast<const unsigned char*>( &num );

        std::string encoded = base64_encode( reinterpret_cast<const unsigned char*>( bytes ), sizeof( num ) );

        std::string encoded1 = base64_encode( reinterpret_cast<const unsigned char*>( sample.c_str() ), sample.size());
